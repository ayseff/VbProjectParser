...
using VbProjectParserCore.Data.ABNF;
...
public class ProjectStream
{
    ...
    public void SetValuesFromStr(string str)
    {
        ...
        if (!canParse)
        {
            ...
            throw new InvalidOperationException($"Could not parse ABNF Syntax. Input was: {str}. Mapped regex pattern is: {pattern}");
        }
        ...
        if (!_abnfSanityCheckSuccess)
            throw new InvalidOperationException($"ABNF data {syntax} did not re-map to itself; result was {_abnfSanityCheck}");
#endif
