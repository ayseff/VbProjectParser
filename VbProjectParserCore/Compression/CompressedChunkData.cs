...
namespace VbProjectParserCore.Compression
{
    ...
    public class CompressedChunkData
    {
        ...
        public CompressedChunkData(CompressedChunkHeader header, XlBinaryReader Data)
        {
            ...
            if (header.CompressedChunkFlag == 0x00)
            {
                ...
                Decompress = (buffer, state) => DecompressRawChunk(buffer, state, this.Data);


            }
            else if (header.CompressedChunkFlag == 0x01)
            {
                ...
                Decompress = (buffer, state) => DecompressTokenSequence(buffer, state, tokenSequences);
            }
            else
            {
                throw new Exception();
