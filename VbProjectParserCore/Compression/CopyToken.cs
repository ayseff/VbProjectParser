...
public class CopyToken : Token
{
    ...
    {
        var info = UnpackCopyToken(Token, state.DecompressedCurrent, state.DecompressedChunkStart);
    ...
    {
        if (ByteCount <= 0)
            throw new ArgumentOutOfRangeException(nameof(ByteCount), ByteCount, "ByteCount must be greater than 0");
    ...
    public static UnpackedInfo UnpackCopyToken(ushort Token, int DecompressedCurrent, int DecompressedChunkStart)
    {
        ...
        ushort temp2 = (ushort)(16 - help.BitCount);
    ...
    {
        return UnpackCopyToken(AsUInt16(), DecompressedCurrent, DecompressedChunkStart);
    }
